...
function hulkappsShowCartDiscounts(discounts) {
  ...
  discounts.cart.items.forEach(function(item) {
    ...
    if (item.original_price_format) item.original_price_format = item.original_price_format.replace("{{amount}}", (item.original_price/100).toFixed(2));
    if (item.discounted_price_format) item.discounted_price_format = item.discounted_price_format.replace("{{amount}}", (item.discounted_price/100).toFixed(2));
    if (item.original_line_price_format) item.original_line_price_format = item.original_line_price_format.replace("{{amount}}", (item.original_line_price/100).toFixed(2));
    
    if (item.discounted_price < item.original_price) {
      flag=1;
      jQuery(".hulkapps-cart-item-price[data-key='" + item.key + "']").html("<span class='original_price money' style='text-decoration:line-through;'>" + item.original_price_format + "</span><br/>" + "<span class='discounted_price money'>" + item.discounted_price_format + "</span>");
      jQuery(".hulkapps-cart-item-line-price[data-key='" + item.key + "']").html("<span class='original_price money' style='text-decoration:line-through;'>" + item.original_line_price_format + "</span><br/>" + "<span class='discounted_price money'>" + item.discounted_line_price_format + "</span>")
    }
    else{
      jQuery(".hulkapps-cart-item-price[data-key='" + item.key + "']").html("<span class='original_price money'>" + item.original_price_format + "</span>");
      jQuery(".hulkapps-cart-item-line-price[data-key='" + item.key + "']").html("<span class='original_price money'>" + item.original_line_price_format + "</span>")            
    }        
  });
  if(flag==1){
    jQuery(".hulkapps-cart-original-total").html(discounts.original_price_total).css("text-decoration", "line-through");
    jQuery("<span class='hulkapps-cart-total money'>" + discounts.discounted_price_total + "</span>").insertAfter('.hulkapps-cart-original-total');        
  ...
  jQuery(".hulkapps-cart-original-total").addClass('money');
